...
export class ShoppingCartService {
  ...
  public constructor(private storageService: StorageService,
    ...
    this.shoppingItemService.getShoppingItems().then(shoppingItems=>this.shoppingItems=shoppingItems);
  ...
  public addItem(ShoppingItem: ShoppingItem, quantity: number): void {
    const cart = this.retrieve();
    let item = cart.items.find((p) => p.shoppingItemName === ShoppingItem.name);
    console.log(cart.items.find((p) => p.shoppingItemName === ShoppingItem.name));
    
    if (item === undefined) {
      item = new CartItem();
      item.shoppingItemName= ShoppingItem.name;
  ...
  private calculateCart(cart: ShoppingCart): void {
    cart.itemsTotal = cart.items
                          .map((item) => item.quantity * this.shoppingItems.find((p) => p.name=== item.shoppingItemName).price)
