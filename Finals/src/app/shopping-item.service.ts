...
export class ShoppingItemService {
  ...
  private shoppingitemsUrl = 'api/shoppingitem';  // URL to web api
  private sortUrl='api/shoppingitem';
  //shoppingitemsUrl: string = 'http://localhost:3000/stickies';
  ...
  sortShoppingItems(): Observable<ShoppingItem[]>{
    return this.http.get<ShoppingItem[]>(this.sortUrl)
    .pipe(
  ...
  // sortShoppingItems(arr: ShoppingItem[], args: any): ShoppingItem[]{
  //   // return this.http.get<ShoppingItem[]>(this.sortUrl)
  //   // .pipe(
  //   //   tap(shoppingitems => this.log(`fetched shoppingitems`)),
  //   //   catchError(this.handleError('getShoppingItems', []))
...
searchShoppingItems(term: string): Observable<ShoppingItem[]> {
  ...
  return this.http.get<ShoppingItem[]>(`api/shoppingitems/?name=${term}`).pipe(
